...
- a **heatmap**, a graph that represents a matrix of values as colors, tipically used in scientific articles related with gene expression data (microarray or RNA-seq data)
...
<a href="http://bioinformatica.uab.cat/base/documents/sgbcursos/documents/heatmap2020_10_13D14_37_15.png" target="new">
    <img src="http://bioinformatica.uab.cat/base/documents/sgbcursos/documents/heatmap2020_10_13D14_37_15.png"  width="30%">
...
**Heatmaps** are a tool of data visualization broadly widely used with biological data. The concept is to represent a matrix of values as colors where usually is organized by a gradient. We can find a large number of these graphics in scientific articles related with gene expression, such as microarray or RNA-seq data.
...
```{r, eval = T}
# Load data
expressionData <- read.csv(file = "https://jcoliver.github.io/learn-r/data/GSE68849-expression.csv")
```
...
We want a heatmap where the different subjects are shown along the `x`-axis, the genes are shown along the `y`-axis, and the shading of the cell reflects how much each gene is expressed within a subject, distinguishing control and infected, usign a color gradient. To this end, we need our data formatted so we have a column corresponding to each of these four dimensions. That is, from the original wide format:
...
GSM1684095 |influenza |IFNA13 |107.219
GSM1684095 |control | IFNA2 |195.175
GSM1684096 |influenza| IFNA5 |10096.47
GSM1684096 |control | IFNA13 |18974.16
GSM1684096 |influenza| IFNA2 |24029.11
... |... |...|...
...
exp.long <- reshape2::melt(
  expressionData,
  id.vars = c("subject", "treatment"),
  variable.name = "gene",
...
<i class="fa fa-key"></i> Use the `gather` or  `melt` functions from the `tidyr` or `reshape2` packages.
...
```{r}
# Basic ggplot
heatmap <- ggplot(exp.long, aes(subject, gene, fill = log.expression)) +
  geom_tile()
heatmap
```
...
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: the overexpressed genes in the infected group are `INFW1`, `IFNA16`, `IFNA2`, `IFNA13`, and `IFNA5`.
&emsp;
...
This activity has been adapted from this [tutorial](https://github.com/pcgoddard/Burchardlab_Tutorials/wiki/GGplot2-Manhattan-Plot-Function) and this [tutorial](http://www.danielroelfs.com/coding/manhattan_plots/).

<i class="fas fa-info-circle"></i> Because the data frame is huge an can affect the R performance, you can load a smaller simulated GWAS dataset to perform this exercise available [here](http://bioinformatica.uab.cat/base/documents/sgbcursos/documents/gwasSample2019_9_23D12_42_50.txt). **I STRONGLY RECOMMEND USING THIS DATASET IF YOU'RE USING A LAPTOP!**

```{r}
# Load the summary statistics file
#gwasData <- read.table("http://bioinformatica.uab.cat/base/continguts/documents/documents.asp?link=documents\sgbcursos\documents\electrocardiographicGWAS2019_9_23D12_42_31.txt", header = TRUE) 

gwasData <- read.table("http://bioinformatica.uab.cat/base/documents/sgbcursos/documents/gwasSample2019_9_23D12_42_50.txt", header = TRUE) 
```
...
manhattanPlot <- ggplot(gwasData, aes(BPcum, -log10(P), colour = factor(CHR))) +
  geom_point()
manhattanPlot
```
...
5. Add a custom palette. We can use a custom palette (`mypalette`). For that, we have to use the `scale_color_manual` function, because we will manually set the colours. Because the palette has 5 colours but we have 10 chromosmes, we are going to use simple R syntax to repeat the palette.
...
```{r}
# Run it!
manhattanPlot <- manhattanPlot + geom_label_repel(data=gwasData[gwasData$P<sig,], aes(label=as.factor(SNP), alpha=0.7), size=5, force=1.3, colour = "black")
manhattanPlot
```

**Can you explain what is the previous function doing? Which data are we using in the `geom_label_repel`?**
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: the previous function is labelling the significant SNPs with their name; in fact the data we are using are only the SNPs which have a P-value which is lower than the `sig` object we created earlier.
&emsp;
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: teh significant SNPs are `rs89152328`, `rs95077576`, and `rs1967363`
&emsp;
...
**Are there statstical differences in the tooth growth when comparing the two delivery methods?**

<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: yes
&emsp;
...
ggplot(ToothGrowth, aes(as.factor(dose), len)) +
  geom_boxplot() +
  geom_signif(comparisons = list(c("0.5", "1")), map_signif_level=FALSE, y_position = 28.5) +
  geom_signif(comparisons = list(c("1", "2")), map_signif_level=FALSE, y_position = 34.5) +
  geom_signif(comparisons = list(c("0.5", "2")), map_signif_level=FALSE, y_position = 40)
```
...
ggplot(ToothGrowth, aes(as.factor(dose), len)) +
  ...
  geom_signif(comparisons = list(c("0.5", "1")), map_signif_level=FALSE, y_position = 28.5) +
  geom_signif(comparisons = list(c("1", "2")), map_signif_level=FALSE, y_position = 34) +
  geom_signif(comparisons = list(c("0.5", "2")), map_signif_level=FALSE, y_position = 40) +
  geom_jitter() +
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: Yes, it's clear that the bigger the dose the higher the tooth growth.
&emsp;
</div>
